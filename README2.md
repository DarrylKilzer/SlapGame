...
- Since our game will have multiple items, we need to find an easy way to access them. 
 ...
 to loop over the entire collection when we are looking for a specific item. What if we instead, create an object that uses the name of the item as a property (for more on this topic, review the ![dictionaries](http://bcw-cheatsheets.herokuapp.com/cheatsheets/1) cheetsheet)?
